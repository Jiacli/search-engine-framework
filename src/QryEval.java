...
public class QryEval {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (!params.containsKey("retrievalAlgorithm")) {
        System.err.println("Error: Parameter 'retrievalAlgorithm' was missing.");
        System.exit(1);
    }
    ...
    if (!params.containsKey("trecEvalOutputPath")) {
        System.err.println("Error: Parameter 'trecEvalOutputPath' was missing.");
        System.exit(1);
    }
    ...
    try {
        br = new BufferedWriter(new FileWriter(new File(params.get("trecEvalOutputPath"))));
    } catch (Exception e) {
        e.printStackTrace();
    }
   ...
   */
  static void writeResultToFile(BufferedWriter br, String QryID, QryResult result, boolean isRankedModel) throws IOException {
      
      if (result.docScores.scores.size() < 1 && result.invertedList.postings.size() < 1) {
          ...
          if (result.invertedList.postings.isEmpty()) {
              ...
              if (isRankedModel) {
                  for (int i = 0; i < bound; i++) {
                      br.write(QryID + " Q0 " + getExternalDocid (result.docScores.getDocid(i))
                         + " " + (i+1) + " " + result.docScores.getDocidScore(i) + " Run\n");
                  }
              }
              else {
                  for (int i = 0; i < bound; i++) {
              ...
              if (isRankedModel) {
                  for (int i = 0; i < bound; i++) {
                      br.write(QryID + " Q0 " + getExternalDocid (result.invertedList.getDocid(i))
                         + " " + (i+1) + " " + result.invertedList.getTf(i) + " Run\n");
                  }
              }
              else {
                  for (int i = 0; i < bound; i++) {
  ...
  /**
   *  Score list output sorting comparator - Ranked model
   *  - since getExternalDocid() is very time consuming, so I cached the id when first use.
   */
  static class entryComparatorRk implements Comparator<ScoreList.ScoreListEntry> {
  ...
  /**
   *  Score list output sorting comparator - Unranked model
   *  - since getExternalDocid() is very time consuming, so I cached the id when first use.
   */
  static class entryComparatorUrk implements Comparator<ScoreList.ScoreListEntry> {
      public int compare(ScoreList.ScoreListEntry o1, ScoreList.ScoreListEntry o2) {
          int rtn = 0;
  ...
  /**
   *  Inverted list output sorting comparator - Ranked model
   *  - sort by External DocID
   *  - since getExternalDocid() is very time consuming, so I cached the id when first use.
   */
  ...
  /**
   *  Inverted list output sorting comparator - Unranked model
   *  - sort by External DocID
   *  - since getExternalDocid() is very time consuming, so I cached the id when first use.
   */
      ...
      if (token.matches("[ ,(\t\n\r]")) {
          ...
          for (String str : term_field) {
              if (token.endsWith("." + str)) {
                  hasField = true;
                  String term = token.substring(0, token.length() - str.length() - 1);
